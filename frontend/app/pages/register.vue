...
async function register() {
    ...
    try {
        ...
        if (err?.data?.detail?.length) {
            error.value = err.data.detail[0].msg
        } else {
...
<template>
    <section class="w-full min-h-screen p-5 flex flex-col justify-center items-center gap-12">
        ...
        <form class="w-full max-w-xl flex flex-col gap-10" @submit.prevent="register">
            ...
            <button type="submit" class="w-full p-2 bg-green-400 text-white font-medium rounded-md transition duration-200 hover:bg-green-500 hover:ring-4 hover:ring-green-400 hover:ring-opacity-20 focus:outline-none focus:bg-green-500 active:scale-95">
                Register
            </button>
        </form>
    </section>
